...
const Login = () => {
  ...
  const { user } = useUserContext();  //CONTEXT TO STORE LOGGED IN USER
  const [sign, setSign] = useState(true); //MANAGE FORM 
  const [err, setErr] = useState(null); //DISPLAY ERROR
  ...
  const handleClick = async (e) => {
    ...
    const errMessage = validate(email.current.value, pass.current.value);
    setErr(errMessage);
